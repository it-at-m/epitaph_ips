import 'dart:collection';
import 'package:flutter_test/flutter_test.dart';
import 'package:epitaph_ips/epitaph_graphs/graphs/graph.dart';
import 'package:epitaph_ips/epitaph_graphs/nodes/edge.dart';
import 'package:epitaph_ips/epitaph_graphs/nodes/simple_vertex.dart';
import 'package:epitaph_ips/epitaph_graphs/nodes/undirected_edge.dart';
import 'package:epitaph_ips/epitaph_graphs/nodes/vertex.dart';
import 'package:epitaph_ips/epitaph_graphs/path_finding/path.dart';

class GraphTests {
  Future<void> runTests() async {
    setUpAll(() {});
    tearDownAll(() {});

    Map<SimpleVertex, List<_MockEdge>> testMap = {
      SimpleVertex('1'): [_MockEdge(SimpleVertex('1'), SimpleVertex('2'), 1)],
      SimpleVertex('2'): [
        _MockEdge(SimpleVertex('2'), SimpleVertex('3'), 1),
        _MockEdge(SimpleVertex('2'), SimpleVertex('4'), 1)
      ],
      SimpleVertex('3'): [_MockEdge(SimpleVertex('3'), SimpleVertex('4'), 1)],
      SimpleVertex('4'): [
        _MockEdge(SimpleVertex('4'), SimpleVertex('5'), 1),
        _MockEdge(SimpleVertex('4'), SimpleVertex('6'), 1)
      ],
      SimpleVertex('5'): [
        _MockEdge(SimpleVertex('5'), SimpleVertex('9'), 1),
        _MockEdge(SimpleVertex('5'), SimpleVertex('8'), 1)
      ],
      SimpleVertex('6'): [_MockEdge(SimpleVertex('6'), SimpleVertex('7'), 1)],
      SimpleVertex('7'): [_MockEdge(SimpleVertex('7'), SimpleVertex('8'), 1)],
      SimpleVertex('8'): [],
      SimpleVertex('9'): [_MockEdge(SimpleVertex('9'), SimpleVertex('10'), 1)],
      SimpleVertex('10'): []
    };

    _MockGraph graph = _MockGraph(testMap);

    //Run all Edge Constructor unit tests
    group("*Graph Constructor Unit Tests*", () {});
    //Run all Edge Method unit tests
    group("*Graph Method Unit Tests*", () {
      test("getVertex in graph", () {
        //Arrange
        String expectedValue = "2";
        Vertex vertex = SimpleVertex("2");

        //Act
        Vertex retrieved = graph.getVertex(vertex);

        //Assert
        expect(retrieved.id, expectedValue);
      });
      test("getVertexByString in graph", () {
        //Arrange
        String expectedValue = "2";

        //Act
        Vertex retrieved = graph.getVertexByString("2");

        //Assert
        expect(retrieved.id, expectedValue);
      });
      test("Edge equalToEdge test", () {
        //Arrange
        List<SimpleVertex> expectedValue = [
          SimpleVertex('1'),
          SimpleVertex('2'),
          SimpleVertex('3'),
          SimpleVertex('4'),
          SimpleVertex('5'),
          SimpleVertex('6'),
          SimpleVertex('7'),
          SimpleVertex('8'),
          SimpleVertex('9'),
          SimpleVertex('10')
        ];

        //Act
        List<Vertex> retrieved = graph.getAllVertices();

        //expected
        //Assert Vertex names should be equal, but vertices have different hashcodes
        for (int i = 0; i < expectedValue.length; i++) {
          expect(retrieved[i].id, expectedValue[i].id);
          expect(
              retrieved[i].hashCode, isNot(equals(expectedValue[i].hashCode)));
        }
      });
      test("getEdge in graph", () {
        //Arrange
        Vertex source = graph.getVertexByString("2");
        Vertex target = graph.getVertexByString("3");
        Edge expectedValue = graph.getEdgeByString("2", "3");

        //Act
        Edge retrieved = graph.getEdge(source, target);

        //Assert
        expect(retrieved, expectedValue);
      });
      test("getEdgeByString in graph", () {
        //Arrange
        Vertex source = graph.getVertexByString("2");
        Vertex target = graph.getVertexByString("3");
        Edge expectedValue = graph.getEdge(source, target);

        //Act
        Edge retrieved = graph.getEdgeByString("2", "3");

        //Assert
        expect(retrieved, expectedValue);
      });
      test("getAllEdges in graph", () {
        //Arrange
        List<Edge> expected = [
          _MockEdge(SimpleVertex('1'), SimpleVertex('2'), 1),
          _MockEdge(SimpleVertex('2'), SimpleVertex('3'), 1),
          _MockEdge(SimpleVertex('2'), SimpleVertex('4'), 1),
          _MockEdge(SimpleVertex('3'), SimpleVertex('4'), 1),
          _MockEdge(SimpleVertex('4'), SimpleVertex('5'), 1),
          _MockEdge(SimpleVertex('4'), SimpleVertex('6'), 1),
          _MockEdge(SimpleVertex('5'), SimpleVertex('9'), 1),
          _MockEdge(SimpleVertex('5'), SimpleVertex('8'), 1),
          _MockEdge(SimpleVertex('6'), SimpleVertex('7'), 1),
          _MockEdge(SimpleVertex('7'), SimpleVertex('8'), 1),
          _MockEdge(SimpleVertex('9'), SimpleVertex('10'), 1),
        ];

        //Act
        List<Edge> retrieved = graph.getAllEdges();

        //Assert Vertex names should be equal, but vertices have different hashcodes
        for (int i = 0; i < expected.length; i++) {
          expect(retrieved[i].a.id, expected[i].a.id);
          expect(retrieved[i].b.id, expected[i].b.id);
          expect(retrieved[i].weight, expected[i].weight);
          expect(retrieved[i].hashCode, isNot(equals(expected[i].hashCode)));
        }
      });
      test("getEdgeWeight in graph", () {
        //Arrange
        double expectedValue = 1;

        //Act
        double retrieved = graph.getEdgeWeight(
            graph.getVertexByString("2"), graph.getVertexByString("3"));

        //Assert
        expect(retrieved, expectedValue);
      });
      test("getPathWeight in graph", () {
        //Arrange
        double expectedValue = 2.0;
        Queue<SimpleVertex> vertices = Queue.from(
            [SimpleVertex('1'), SimpleVertex('2'), SimpleVertex('3')]);
        Path path = Path(vertices);

        //Act
        double retrieved = graph.getPathWeight(path);

        //Assert
        expect(retrieved, expectedValue);
      });
      test("getOutgoingEdges in graph", () {
        //Arrange
        List<_MockEdge> expected = [
          _MockEdge(SimpleVertex('4'), SimpleVertex('5'), 1),
          _MockEdge(SimpleVertex('4'), SimpleVertex('6'), 1)
        ];

        //Act
        List<Edge> retrieved =
            graph.getOutgoingEdges(graph.getVertexByString("4"));

        //Assert
        expect(retrieved.toString(), expected.toString());
      });
      test("getOutgoingEdgesByString in graph", () {
        //Arrange
        List<_MockEdge> expected = [
          _MockEdge(SimpleVertex('4'), SimpleVertex('5'), 1),
          _MockEdge(SimpleVertex('4'), SimpleVertex('6'), 1)
        ];

        //Act
        List<Edge> retrieved = graph.getOutgoingEdgesByString('4');

        //Assert
        expect(retrieved.toString(), expected.toString());
      });
      test("getNeighboringEdgesOfEdge in graph", () {
        List<_MockEdge> expected = [
          _MockEdge(SimpleVertex('2'), SimpleVertex('4'), 1),
          _MockEdge(SimpleVertex('3'), SimpleVertex('4'), 1)
        ];

        //Act
        List<Edge> retrieved = graph.getNeighboringEdgesOfEdge(
          graph.getEdgeByString(
            SimpleVertex('2').id,
            SimpleVertex('3').id,
          ),
        );

        //Assert
        expect(retrieved.toString(), expected.toString());
      });
      test("getNeighboringEdgesOfEdgeByString in graph", () {
        List<_MockEdge> expected = [
          _MockEdge(SimpleVertex('2'), SimpleVertex('4'), 1),
          _MockEdge(SimpleVertex('3'), SimpleVertex('4'), 1)
        ];

        //Act
        List<Edge> retrieved =
            graph.getNeighboringEdgesOfEdgeByString('2', '3');

        //Assert
        expect(retrieved.toString(), expected.toString());
      });
      test("toJson test", () {
        //Arrange
        Map<String, dynamic> expectedValue = {
          'graph': {
            '1': {
              'a': {'id': '1'},
              'b': {'id': '2'},
              'weight': 1.0
            },
            '2': {
              'a': {'id': '2'},
              'b': {'id': '4'},
              'weight': 1.0
            },
            '3': {
              'a': {'id': '3'},
              'b': {'id': '4'},
              'weight': 1.0
            },
            '4': {
              'a': {'id': '4'},
              'b': {'id': '6'},
              'weight': 1.0
            },
            '5': {
              'a': {'id': '5'},
              'b': {'id': '8'},
              'weight': 1.0
            },
            '6': {
              'a': {'id': '6'},
              'b': {'id': '7'},
              'weight': 1.0
            },
            '7': {
              'a': {'id': '7'},
              'b': {'id': '8'},
              'weight': 1.0
            },
            '9': {
              'a': {'id': '9'},
              'b': {'id': '10'},
              'weight': 1.0
            }
          }
        };

        //Act
        Map<String, dynamic> retrieved = graph.toJson();

        //expected
        expect(retrieved, expectedValue);
      });
      test("toString Edge syntax test", () {
        //Arrange
        String expectedValue =
            'Graph({SimpleVertex(id: 1): [UndirectedEdge(a: 1, b: 2, weight: 1.0)], SimpleVertex(id: 2): [UndirectedEdge(a: 2, b: 3, weight: 1.0), UndirectedEdge(a: 2, b: 4, weight: 1.0)], SimpleVertex(id: 3): [UndirectedEdge(a: 3, b: 4, weight: 1.0)], SimpleVertex(id: 4): [UndirectedEdge(a: 4, b: 5, weight: 1.0), UndirectedEdge(a: 4, b: 6, weight: 1.0)], SimpleVertex(id: 5): [UndirectedEdge(a: 5, b: 9, weight: 1.0), UndirectedEdge(a: 5, b: 8, weight: 1.0)], SimpleVertex(id: 6): [UndirectedEdge(a: 6, b: 7, weight: 1.0)], SimpleVertex(id: 7): [UndirectedEdge(a: 7, b: 8, weight: 1.0)], SimpleVertex(id: 8): [], SimpleVertex(id: 9): [UndirectedEdge(a: 9, b: 10, weight: 1.0)], SimpleVertex(id: 10): []})';
        //Act
        String retrieved = graph.toString();

        //expected
        expect(retrieved, expectedValue);
      });
    });
  }
}

class _MockGraph extends Graph {
  _MockGraph(this.graph);

  ///This map is the representation of your directed graph.
  final Map<Vertex, List<Edge>> graph;

  @override
  Vertex getVertex(Vertex vertex) =>
      graph.keys.firstWhere((element) => vertex.id == element.id);

  @override
  Vertex getVertexByString(String source) =>
      graph.keys.firstWhere((element) => source == element.id);

  @override
  Edge getEdge(Vertex a, Vertex b) =>
      graph[a]!.firstWhere((element) => element.equalToVertices(a, b));

  @override
  Edge getEdgeByString(String a, String b) {
    Vertex vertexA = getVertexByString(a);
    Vertex vertexB = getVertexByString(b);

    return graph[vertexA]!
        .firstWhere((element) => element.equalToVertices(vertexA, vertexB));
  }

  @override
  double getEdgeWeight(Vertex a, Vertex b) =>
      getEdgeByString(a.id, b.id).weight;

  @override
  List<Edge> getNeighboringEdgesOfEdge(Edge edge) {
    List<Edge> edgeList = [];
    getOutgoingEdgesByString(edge.a.id).forEach((element) {
      edgeList.add(element);
    });

    getOutgoingEdgesByString(edge.b.id).forEach((element) {
      edgeList.add(element);
    });

    edgeList.remove(edge);

    return edgeList;
  }

  @override
  List<Edge> getNeighboringEdgesOfEdgeByString(String a, String b) {
    List<Edge> tmp = [];
    getOutgoingEdgesByString(a).forEach((element) {
      tmp.add(element);
    });

    getOutgoingEdgesByString(b).forEach((element) {
      tmp.add(element);
    });

    tmp.remove(getEdgeByString(a, b));

    return tmp;
  }

  @override
  double getPathWeight(Path path) {
    double pathWeight = 0;
    Queue<Vertex> pathList = path.path;

    for (int i = 0; i < pathList.length - 1; i++) {
      pathWeight +=
          getEdgeWeight(pathList.elementAt(i), pathList.elementAt(i + 1));
    }

    return pathWeight;
  }

  @override
  List<Edge> getAllEdges() {
    List<Edge> tmp = [];
    for (var element in graph.values) {
      tmp.addAll(element);
    }

    return tmp;
  }

  @override
  List<Vertex> getAllVertices() {
    List<Vertex> vertexList = [];
    for (var element in graph.keys) {
      vertexList.add(element);
    }
    return vertexList;
  }

  Map<String, dynamic> toJson() {
    // Lists cannot be automatically be generated, this is why this method is convoluted
    Map<String, dynamic> pointsMap = {};
    graph.forEach((k, v) {
      for (var element in v) {
        pointsMap[k.id] = element.toJson();
      }
    });

    return {'graph': pointsMap};
  }

  @override
  String toString() => 'Graph($graph)';

  @override
  List<Edge> getOutgoingEdges(Vertex vertex) {
    {
      List<Edge> tmp = [];
      tmp.addAll(graph[vertex]!);

      return tmp;
    }
  }

  @override
  List<Edge> getOutgoingEdgesByString(String source) {
    List<Edge> tmp = [];
    tmp.addAll(graph[getVertexByString(source)]!);
    return tmp;
  }
}

class _MockEdge extends UndirectedEdge {
  _MockEdge(Vertex a, Vertex b, double weight) : super(a, b, weight);

  @override
  UndirectedEdge copy() => this;

  @override
  Map<String, dynamic> toJson() =>
      {'a': a.toJson(), 'b': b.toJson(), 'weight': weight};
}
